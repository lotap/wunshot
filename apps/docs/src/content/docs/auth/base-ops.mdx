 ...
 */
export async function signUp({ input }: { input: unknown }) {
  ...
  try {
    ...
    const {
      ...
      const [session, access] = await Promise.all([
        createSession({ userId, ipAddress }, { tx, label: "sign_up" }),
 ...
 *   add more robust rate-limiting using in-memory storage
 */
export async function signIn({ input }: { input: unknown }) {
  /** Apply a label for the activity to the logger */
  const { logSuccess, logFailure } = initializeLoggers({
    label: ACTIVITY_LABEL,
  });
  ...
  try {
    ...
    const user = await selectUserAndSessionsByUsername({ username });
    ...
    await logSuccess({ userId, guestsArchiveId: guestsArchive?.id });
...
export async function refreshAuth({ token }: { token: string }) {
  /** Apply a label and ipAddress for the activity to the logger */
  const { logSuccess, logFailure } = initializeLoggers({
    label: ACTIVITY_LABEL,
  });
