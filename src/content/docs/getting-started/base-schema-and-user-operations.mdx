...
:::caution
From this point on, the documentation will assume you have read and completed all of the steps on the [installation page](/wunshot/getting-started/installation).
:::
...
Paste into `@/db/operations/user.ts`

```ts title="@/db/operations/user.ts"
import "server-only"; // <- if you are using react server components
...
export async function insertUser() {
  return preparedInsertUser.execute();
}
