...
:::danger
This page is out of date and will be updated to reflect the current conventions soon.
:::

## Understanding the options

A session is a collection of user data that persists over the use of your application. Session authorization relies on the stored information to determine the level of access that user should have.

**wunshot provides two main approaches for storing session auth data: cookies and your database.**
...
- Retrieving information from the database can be expensive in time and bandwidth.
  ...
  - A bad actor could use those lookups as an attack vector - either for (D)DOS or to drive up the costs of running the app.
...
:::note
This will be the assumed default moving forward with wunshot. I will probably rewrite this page to make it more clear.
:::

A hybrid approach entails using a database for an initial authentication, but trusting short-lived cookies for subsequent authentication done in the same tab/window.

This approach maximizes flexibility while minimizing the amount of requests to the database. It's similar conceptually to [`stale-while-revalidate`](https://web.dev/articles/stale-while-revalidate#whats_it_mean), but it using cookies for the cache.

It's also similar to a standard cookies-only approach. The key difference is that it gives you the ability to force revalidation in certain areas of your app. That means a high level of security can be achieved without sacrificing convenience for the user.
