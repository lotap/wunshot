...
| `nonce_hash` | `text`       | [Hashed random value used for verification](#what-is-a-refreshtoken) |
...
[There is no mechanism to revoke an `accessToken`](#why-use-a-database).
...
The `nonce` is used like a "password" to the session.
...
The main benefit of JWTs over other protocols is their wide adoption.
...
So it can be slow, expensive, and hard to scale connections as the amount of users grows.
